...
# Deploy Full Magistrala Platform to EC2
# This script deploys the complete Magistrala IoT platform

set -e

EC2_IP="44.196.96.48"
DB_HOST="magistrala-iot-postgres.cwxucmeki4tt.us-east-1.rds.amazonaws.com"
DB_USER="postgres"
DB_PASS="magistrala123"
DB_NAME="magistrala"

echo "🚀 Deploying Full Magistrala Platform to EC2..."
echo "📋 Target: $EC2_IP"

# Create the deployment script
cat > magistrala-deployment.sh << 'EOF'
#!/bin/bash
...
MG_POSTGRES_HOST=DB_HOST_PLACEHOLDER
MG_POSTGRES_PORT=5432
MG_POSTGRES_USER=DB_USER_PLACEHOLDER
MG_POSTGRES_PASS=DB_PASS_PLACEHOLDER
MG_POSTGRES_DB=DB_NAME_PLACEHOLDER
...
sed -i "s/DB_HOST_PLACEHOLDER/$1/g" .env
sed -i "s/DB_USER_PLACEHOLDER/$2/g" .env
sed -i "s/DB_PASS_PLACEHOLDER/$3/g" .env
sed -i "s/DB_NAME_PLACEHOLDER/$4/g" .env
...
sed "s/DB_HOST_PLACEHOLDER/$DB_HOST/g" magistrala-deployment.sh | \
sed "s/DB_USER_PLACEHOLDER/$DB_USER/g" | \
sed "s/DB_PASS_PLACEHOLDER/$DB_PASS/g" | \
sed "s/DB_NAME_PLACEHOLDER/$DB_NAME/g" > magistrala-deployment-final.sh

echo "📤 Uploading deployment script to EC2..."

# Note: In a real deployment, you would use SCP or AWS Systems Manager
# For this demo, we'll simulate the deployment
echo "🎯 Deployment script ready for EC2 execution"
echo "📋 Script: magistrala-deployment-final.sh"
echo "🌐 Target: $EC2_IP"
echo ""
echo "To execute on EC2:"
echo "1. Copy script to EC2 instance"
echo "2. Run: chmod +x magistrala-deployment-final.sh"
echo "3. Execute: ./magistrala-deployment-final.sh $DB_HOST $DB_USER $DB_PASS $DB_NAME"
echo ""
echo "✅ Full Magistrala deployment package ready!"